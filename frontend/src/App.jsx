...
function Reports({ api }) {
  ...
  return <section className="page reports-page">
    ...
    <div className="report-kind"><label>Report<select value={kind} onChange={(e) => { setKind(e.target.value); setPage(1); setFilters(emptyReportFilters) }}>{reportKinds.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label></div>
    <div className="report-project-filter"><ProjectPicker api={api} projects={projects} value={filters.project_id} onChange={(value) => setFilter('project_id', value)} allOption /></div>
    {overview && <div className="period-control" role="group" aria-label="Report period">{[['today','Today'],['week','This Week'],['month','This Month'],['lifetime','Lifetime']].map(([value, text]) => <button key={value} className={period === value ? 'active' : ''} onClick={() => setPeriod(value)}>{text}</button>)}</div>}
    <div className="filters"><select aria-label="Project filter" value={filters.project_id} onChange={(e) => setFilter('project_id', e.target.value)}><option value="">All projects</option>{projects.map((project) => <option value={project.id} key={project.id}>{project.project_code} - {project.project_name}</option>)}</select>{(overview || kind === 'recurring-fees') && <select aria-label="Fee type filter" value={filters.fee_type} onChange={(e) => setFilter('fee_type', e.target.value)}><option value="">All fee types</option>{feeTypes.map((type) => <option key={type}>{type}</option>)}</select>}{kind === 'domain-billing' && <><select aria-label="Domain payment status" value={filters.payment_status} onChange={(e) => setFilter('payment_status', e.target.value)}><option value="">All payment states</option>{['Not Priced','Unpaid','Partially Paid','Paid'].map((status) => <option key={status}>{status}</option>)}</select><select aria-label="Domain purchase status" value={filters.purchase_status} onChange={(e) => setFilter('purchase_status', e.target.value)}><option value="">All purchase states</option>{['Not Purchased','Active','Expired','Cancelled'].map((status) => <option key={status}>{status}</option>)}</select></>}{kind === 'expense' && <><select value={filters.expense_category} onChange={(e) => setFilter('expense_category', e.target.value)}><option value="">All expense categories</option>{expenseCategories.map((category) => <option key={category}>{category}</option>)}</select><select value={filters.payment_method} onChange={(e) => setFilter('payment_method', e.target.value)}><option value="">All payment methods</option>{paymentMethods.map((method) => <option key={method}>{method}</option>)}</select></>}{kind === 'payment-collection' && <select value={filters.financial_account_id} onChange={(e) => setFilter('financial_account_id', e.target.value)}><option value="">All receivers</option>{financialAccounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select>}{kind === 'invoice' && <><select value={filters.invoice_type} onChange={(e) => setFilter('invoice_type', e.target.value)}><option value="">All invoice types</option>{invoiceTypes.map((type) => <option key={type}>{type}</option>)}</select><select value={filters.status} onChange={(e) => setFilter('status', e.target.value)}><option value="">All invoice statuses</option>{['Draft','Sent','Partially Paid','Paid','Overdue','Cancelled'].map((status) => <option key={status}>{status}</option>)}</select></>}{kind === 'recurring-fees' && <select value={filters.status} onChange={(e) => setFilter('status', e.target.value)}><option value="">All fee statuses</option>{['Not Due','Due Soon','Due Today','Overdue','Paid','Cancelled'].map((status) => <option key={status}>{status}</option>)}</select>}{['project-financial','outstanding-balance','profit'].includes(kind) && <><select value={filters.status} onChange={(e) => setFilter('status', e.target.value)}><option value="">All project statuses</option>{projectStatuses.map((status) => <option key={status}>{status}</option>)}</select><select value={filters.payment_status} onChange={(e) => setFilter('payment_status', e.target.value)}><option value="">All payment statuses</option>{paymentStatuses.map((status) => <option key={status}>{status}</option>)}</select></>}{!overview && <><input aria-label="Date from" type="date" value={filters.date_from} onChange={(e) => setFilter('date_from', e.target.value)} /><input aria-label="Date to" type="date" value={filters.date_to} onChange={(e) => setFilter('date_to', e.target.value)} /></>}{overview && data.date_from && <span className="date-range">{data.date_from} to {data.date_to}</span>}<button onClick={() => { setFilters(emptyReportFilters); setPage(1) }}>Clear</button></div>
    {error && <div className="alert">{error}</div>}
...
function Table({ rows, columns, actions }) {
  ...
  return <div className="table-wrap"><table className="data-table"><thead><tr>{columns.map((col) => <th key={col.key}>{col.label || label(col.key)}</th>)}{actions && <th className="action-column">Actions</th>}</tr></thead><tbody>{rows.map((row) => <tr key={`${row.id}-${row.invoice_number || row.receipt_number || row.project_code || ''}`}>{columns.map((col) => <td key={col.key} data-label={col.label || label(col.key)}>{renderCell(row[col.key], col.key)}</td>)}{actions && <td className="actions action-column">{actions(row)}</td>}</tr>)}</tbody></table></div>
}
