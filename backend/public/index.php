...
function project_list(): array
{
    if (($_GET['compact'] ?? '') === '1') {
        ...
        return select_all('SELECT p.id, p.project_code, p.project_name, p.project_type, p.customer_company_name,
            ...
            ROUND(COALESCE((SELECT SUM(pay.amount) FROM payments pay WHERE pay.project_id=p.id AND pay.payment_scope="Project"),0),2) total_paid,
...
function validate_resource_data(string $resource, array $data, array $requiredFields): array
{
    ...
    if ($resource === 'invoices') {
        ...
        foreach (['discount_amount','tax_amount','paid_amount','project_total_amount','previously_paid_amount','remaining_project_amount'] as $field) valid_decimal($errors, $data, $field, true);
        valid_decimal($errors, $data, 'total_amount');
        max_length($errors, $data, 'header_note', 500);
    ...
    if (isset($crud[$resource])) {
        ...
        if ($method === 'POST' || ($method === 'PUT' && $id)) {
            ...
            if ($resource === 'invoices') {
                ...
                $data['items'] = $items;
                foreach (['discount_amount','tax_amount','paid_amount','project_total_amount','previously_paid_amount','total_amount','remaining_project_amount'] as $field) $data[$field] = money($data[$field] ?? 0);
